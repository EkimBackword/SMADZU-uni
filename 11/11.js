...
const s = 2;                //  Размер массива КА (без учета двух замыкающих ячеек)
const m = 12;               //  Количество ячеек гистограммы
const d = 2;                //  Размерность пространства
...
function KA_random() {
  ...
  for (let j = 1; j <= s; j++) {    //Копирование новых значений из временного массива
    A[j] = B[j];
  }
  A[0]     = A[s];                  //Замыкание границ массива
  A[s + 1] = A[1];
  ...
  let x = 0;
  for (let j = 1; j <= s; j++) {
    x = x + A[j]*Math.exp((j - 1 - s)*Math.log(2));
